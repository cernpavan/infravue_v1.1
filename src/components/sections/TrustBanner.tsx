...
export default function TrustBanner() {
  return (
    <section className="bg-cream overflow-hidden py-10 border-b border-sand/20">
      ...
      <div className="relative flex overflow-hidden group">
        {/* Fading edges matching the off-white background */}
        <div className="pointer-events-none absolute left-0 top-0 z-10 h-full w-12 lg:w-28 bg-gradient-to-r from-cream to-transparent" />
        <div className="pointer-events-none absolute right-0 top-0 z-10 h-full w-12 lg:w-28 bg-gradient-to-l from-cream to-transparent" />
        ...
        <div className="flex gap-4 px-4 w-max animate-marquee hover:[animation-play-state:paused] pb-4">
          {[...TRUST_ITEMS, ...TRUST_ITEMS, ...TRUST_ITEMS].map((item, index) => {
            ...
            return (
              <div
                key={index}
                className="flex flex-col items-center justify-center gap-3 bg-[#FCF9F4] px-4 py-6 rounded-xl border border-sand/30 shadow-sm hover:-translate-y-1 hover:bg-white hover:shadow-[0_12px_32px_rgba(30,58,106,0.1)] hover:border-sand/50 transition-all duration-300 group/card shrink-0 w-[180px] md:w-[220px]"
              >
