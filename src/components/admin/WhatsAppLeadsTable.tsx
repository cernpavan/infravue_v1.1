...
import EditWhatsAppLeadModal from "./EditWhatsAppLeadModal";
...
export default function WhatsAppLeadsTable({ initialLeads }: WhatsAppLeadsTableProps) {
  const [leads, setLeads] = useState<WhatsAppLead[]>(initialLeads);
  ...
  const getCountByPeriod = (leads: WhatsAppLead[], days?: number) => {
    const now = new Date();
  ...
  const filteredLeads = useMemo(() => {
    let filtered = leads.filter((lead) => {
      ...
      const matchesSearch =
        lead.requestId.toLowerCase().includes(searchLower) ||
  ...
  const handleEdit = (lead: WhatsAppLead) => {
    setSelectedLead(lead);
    setIsEditOpen(true);
  };

  const handleSave = (updatedLead: WhatsAppLead) => {
    setLeads((prev) =>
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const response = await fetch(`/api/admin/whatsapp-leads/${id}`, {
        method: "DELETE",
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            type="text"
            placeholder="Search by request ID, source..."
            value={search}
        ...
        {/* Export */}
        <ExportButtons data={filteredLeads} type="whatsapp" />
      </div>
              ...
              {filteredLeads.length > 0 ? (
                filteredLeads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-mono text-sm text-black">
                      {lead.requestId}
                    </td>
